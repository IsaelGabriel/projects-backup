import pygame
from info import *
from entity import Entity,MovableEntity,obj_list
...
class Player(MovableEntity):
    ...
    def custom_update(self):
        collisions = self.get_collision_list((10**3))
        for c in collisions:
            if c.dir == 'down':
                self.grounded = True
                #print('down!')
        if self.gravity_on and not self.grounded: self.add_force(self.gravity)
        if get_button('jump'): self.check()


    def render(self,camera):
        camera.draw_rect(pygame.Color('green'),self.pos,self.rect.size)

class Mid_Player(MovableEntity):
# start
    def start(self):
        # basic physics
        self.gravity = 250
        ...
        # walking
        self.speed = 500
        self.acceletation = 100
        self.decceleration = 50
        self.direction = 'right'
        # jumping
        self.jump_force = 500
        self.jump_hold = 0.5
        self.holding_jump = False
        self.jump_duration = 0.3 # in seconds
        self.jump_step = 0
        self.jumping = False
...
# update
    def update(self):
        if not self.grounded:
            self.add_velocity(1,float(self.gravity))
            if self.velocity[1] > self.gravity: self.velocity = (self.velocity[0],self.gravity)
        else: self.velocity = (self.velocity[0],0)
        self.move()
        self.jump()
        vel = (self.get_velocity_axis(0), self.get_velocity_axis(1))
    ...
    def move(self):
        ...
        if get_button('w_left'):
            ...
            self.direction = 'right'
            movement = 1
        #print(str(self.speed) + " " +str( movement )+ " " + str(self.deltaTime))
        if movement == 0:
            if self.direction == 'right':
                self.add_velocity(0,-self.decceleration)
            ...
            self.add_velocity(0,self.acceletation*movement)
            if self.velocity[0] > self.speed: self.velocity = (self.speed,self.velocity[1])
            elif self.velocity[0] < -self.speed: self.velocity = (-self.speed,self.velocity[1])

    def jump(self):
        if self.get_collision('down',1) != None: self.grounded = obj_list[self.get_collision('down',1)].__class__.__name__ == 'Solid'
        ...
        if get_button('jump') and not self.holding_jump:
            ...
            if self.grounded and self.jump_step == 0:
                self.jumping = True
                self.jump_hold = 0.6
        elif not get_button('jump'):
            self.holding_jump = False
            self.jump_hold = 0.5

        if self.jumping:
            if self.holding_jump and self.jump_hold > 0.5:
                self.jump_hold += self.deltaTime
            self.velocity = (self.velocity[0],-self.jump_force*self.jump_hold)
            self.jump_step += self.deltaTime
            if self.jump_step >= self.jump_duration or(self.jump_step >= self.jump_duration/5  and self.jump_hold == 0.5):
                self.jump_step = 0
                self.jumping = False
                self.jump_hold = 0.5
...
# render
    def render(self,camera):
        camera.draw_rect(pygame.Color('green'),self.rect)
    ...
    def render(self):
        ...
        if len(self.dash_render) > 0:
            for pos in range(0,len(self.dash_render)):
                ...
                self.scene.camera.draw_rect(pygame.Color(c),rect)
