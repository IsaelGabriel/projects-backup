...
class Player(PhysicsEntity):
    ...
    def custom_update(self):
        c_down = self.get_collision(((self.pos[0]),(self.pos[1]+(self.size[1]/2))),(self.size[0],0.5))
        if c_down != None: self.grounded = (obj_list[c_down].collision == 'on')
        else: self.grounded = False
        #print(c_down)
        if not self.grounded: self.velocity = (4,self.gravity)
        #else: self.velocity = (0,0)
        #if self.get_collision(self.pos,self.size) != None: print('aaa')
        #if c_down != None: print('s')
        

    def render(self,camera):
        camera.draw_rect(pygame.Color('green'),self.pos,self.size)

class MPlayer(PhysicsEntity):
# start
    def start(self):
        # basic physics
        self.gravity = 250
        ...
        # walking
        self.speed = 50#0
        self.acceletation = 10#0
        self.decceleration = 5#0
        self.direction = 'right'
        # jumping
        self.jump_force = 30#0
        self.jump_count = 0
        self.holding_jump = False
        self.jump_duration = 0.500 # in seconds
        self.jump_step = 0
        self.jumping = False
...
# update
    ...
    def jump(self):
        if self.get_collision('down',1) != None: self.grounded = obj_list[int(self.get_collision('down',1).split(":")[1])].__class__.__name__ == 'Solid'
        else: self.grounded = False
