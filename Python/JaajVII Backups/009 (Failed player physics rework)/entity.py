...
from info import get_multiplier
...
class Entity(ABC):
            ...
            if i == 0:
                if self.get_collision(direction,1) != None:
                    ...
                    if direction == 'down':
                        ...
                        pos = (self.rect.left,self.rect.up+1)
                        size = (self.rect.w,1)
                    ...
                    if pygame.Rect(pos,size).colliderect(obj.rect):
                        ...
                        return 0
            elif self.get_collision(direction,i*self.deltaTime) != None:
                return (i-1) * mult
        return vel
    ...
    def apply_velocity(self,axis):
        ...
        if vel >= 0: direction = direction[1]
        else:
            direction = direction[0]
            mult = -1

        for i in range(0,int(abs(vel*self.deltaTime*(10**get_multiplier(self.deltaTime)))+1)):
            if i == 0:
                if self.get_collision(direction,1) != None:
                    obj = obj_list[self.get_collision(direction,1)]
                    pos = (0,0)
                    ...
                    self.velocity = ((self.velocity[0] * int(axis != 0)),(self.velocity[1] * int(axis != 1)))
                    return 0
            elif self.get_collision(direction,i*(10**-get_multiplier(self.deltaTime))) != None:
                return i*(10**-get_multiplier(self.deltaTime)) * mult
        return vel*(10**-get_multiplier(self.deltaTime)) * mult
