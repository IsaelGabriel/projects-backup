...
class Player(Entity):
    def start(self):
        ...
        self.dash_step = 0.0
        self.dash_duration = 0.100 #12 # in frames
        self.dash_render = []
        self.dash_render_count = 0.0
        self.dash_render_frames = 0.050

    def update(self):
        self.grounded =  self.get_every_collision('down',1)
        self.move()
        self.jump()
    ...
    def dash(self):
        if len(self.dash_render) > 0:
            self.dash_render_count += self.scene.deltaTime
        ...
        if self.dashing:
            if int(self.dash_render_count) % 2 == 0: self.dash_render.append((self.pos.x,self.pos.y))
            self.dash_charged = False
            mult = Vector2(0,0)
            ...
            self.dash_step += self.scene.deltaTime
