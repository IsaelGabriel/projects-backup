    ...
    def dash(self):
        ...
        if get_button('dash') and not self.holding_dash and not self.dashing:
            ...
            if self.dash_charged:
                ...
                if get_button('w_left'): self.dash_direction.x = -1
                elif get_button('w_right'): self.dash_direction.x = 1

                if get_button('l_up'): self.dash_direction.y = -1
                elif get_button('l_down'): self.dash_direction.y = 1
                elif not get_button('w_left') and get_button('w_right'):
                     if self.direction == 'left': self.dash_direction.x = -1
                     else: self.dash_direction.x = 1
        ...
        if self.dashing:
            ...
            self.velocity = (self.dash_direction *  self.dash_velocity)
            self.dash_step += 1
